...
knnresult distrAllkNN(double * X, int n, int d, int k)
{
	...
	memcpy(tempY, X, n*d*sizeof(double));
	...
	for (int i=1; i<p; i++)
	{
		//start communication time counting
		double startCommunication = MPI_Wtime();

		//Even processes send first
		//odd processes receive first
		if (id % 2 == 0)
		{
			MPI_Send(Y, n*d, MPI_DOUBLE, dst, 100, MPI_COMM_WORLD);
			MPI_Recv(Y, n*d, MPI_DOUBLE, rcv, 100, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		}
		else
		{
			MPI_Recv(tempY, n*d, MPI_DOUBLE, rcv, 100, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			MPI_Send(Y, n*d, MPI_DOUBLE, dst, 100, MPI_COMM_WORLD);
			memcpy(Y,tempY,n*d*sizeof(double));
		}

		//calculate communication time for the first rotation
		double endCommunication = MPI_Wtime();
		communicationTime = communicationTime + endCommunication - startCommunication;
	...
	double computationTime = endComputation - startComputation - communicationTime;
	double totalTime = endComputation - startComputation;

	printf("ID: %d, totalTime %f s,  computationTime %f s, communicationTime %f s\n", id, totalTime, computationTime, communicationTime);
